#!/usr/bin/env python3
"""
Prepare OpenAI Batch API requests for concept filtering.

This script does not submit the batch. It only creates:
  - a JSONL file with one Batch API request per concept batch
  - a manifest JSON file mapping each request to its concepts

Usage:
    python prepare_openai_batch_requests.py \
        --input output/05_all_concepts.txt \
        --output output/openai_batch \
        --model gpt-5.4
"""
...
log = logging.getLogger("prepare_openai_batch")
...
def build_batch_prompt(batch: list[dict]) -> str:
    """Build the user prompt for one concept batch."""
    concept_list = "\n".join(
...
def load_concepts(input_path: Path) -> list[dict]:
    """Load one concept per line from a text file."""
    entries = []
...
def batch_concepts(entries: list[dict], batch_size: int) -> list[list[dict]]:
    """Split concepts into fixed-size batches."""
    return [
...
def make_batch_request(custom_id: str, model: str, prompt: str) -> dict:
    """Build one OpenAI Batch API request payload."""
    return {
        "custom_id": custom_id,
        "method": "POST",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Prepare OpenAI Batch API requests for concept filtering",
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("output/openai_batch"),
    ...
    parser.add_argument(
        ...
        type=str,
        default="gpt-5.4",
        help="OpenAI model for the batch job",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    with open(requests_path, "w", encoding="utf-8") as requests_file:
        for batch_idx, batch in enumerate(batches, start=1):
            custom_id = f"concept-filter-batch-{batch_idx:04d}"
            prompt = build_batch_prompt(batch)
            request = make_batch_request(custom_id, args.model, prompt)
    ...
    log.info(f"Saved batch requests  → {requests_path}")
