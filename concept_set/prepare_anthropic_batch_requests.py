#!/usr/bin/env python3
"""
Prepare Anthropic Message Batches API requests for concept filtering.

This script does not submit the batch. It creates:
  - a JSON payload for client.messages.batches.create(...)
...
log = logging.getLogger("prepare_anthropic_batch")
...
def make_request(custom_id: str, model: str, prompt: str, max_tokens: int) -> dict:
    return {
        "custom_id": custom_id,
        "params": {
            "model": model,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Prepare Anthropic Message Batch payload for concept filtering",
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("output/anthropic_batch"),
    ...
    parser.add_argument(
        ...
        help="Anthropic model for the batch job",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    payload_path = args.output / "requests.json"
    ...
    for batch_idx, batch in enumerate(batches, start=1):
        ...
        requests.append(
            make_request(custom_id, args.model, build_batch_prompt(batch), args.max_tokens)
        )
    ...
    log.info(f"Saved batch payload   → {payload_path}")
